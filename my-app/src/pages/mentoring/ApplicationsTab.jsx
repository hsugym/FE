...
export default function ApplicationsTab({ userId, userName, darkMode }) {
  ...
  // 내 멘토 글에 신청한 멘티들 (pending)
  // 멘토 글을 쓴 사람이 보는 신청 목록 - mentorUserId로 필터링
  const mentorApplications = matches.filter(
    (m) => myMentorPost && m.mentorUserId === userId && m.status === "pending"
  );

  // 현재 활성화된 매칭 (멘토로서)
  const activeMentorMatch = matches.find(
    (m) => myMentorPost && m.mentorUserId === userId && m.status === "active"
  );

  // 현재 활성화된 매칭 (멘티로서)
  const activeMenteeMatch = matches.find(
    (m) => myMenteePost && m.menteeUserId === userId && m.status === "active"
  );

  // 모달 확인 동작
  const handleConfirm = (type, target) => {
    if (type === "accept") {
      acceptMatch(target.mentorUserId, target.menteeUserId);
    }
    if (type === "reject" || type === "terminate") {
      terminateMatch(target.mentorUserId, target.menteeUserId);
    }
    setModal(null);
  };
              ...
              {mentorApplications.map((application) => {
                ...
                return (
                  <div
                    ...
                    className={`border p-4 rounded-lg ${darkMode ? "bg-gray-700 border-gray-600" : "bg-gray-50 border-gray-200"
                      }`}
                  >
